class Option {
    ...
    global_handlers: Option.Handler<any>[];
    ...
    register<T>( property: Option.Property<T> ) {
        ...
        if ( Option.isSelectProperty( property ) ) {
            ...
            this.map.set( property.key, {
                key, type, value, min, max, handlers,
    ...
    set<T>( key: string, value: T ) {
        ...
        this.global_handlers.forEach(handler => {
                handler(event);
        });
    }
...
interface SliderParam {
    ...
    onui?: (ui: HTMLInputElement, opt: { sliderToValue: (slider: number) => number , valueToSlider: (value:number) => number }) => void;
    label?: string;
}
...
class DomTool {
    ...
    static createSlider(option: SliderParam) {
        const min = option.min;
        const max = option.max;
        const N = 100;
        const sliderToValue = (slider: number) => slider * (max - min) / N + min;
        const valueToSlider = (value: number) => (value - min) / (max - min) * N;
        const pane = document.createElement("div");
        ...
        slider.value = valueToSlider(v).toString();
        pane.title = option.label ? option.label + ":" + v.toString() : v.toString();
        if (option.onchange) {
        ...
        if (option.onui) {
            option.onui(slider, {
                sliderToValue: sliderToValue, valueToSlider: valueToSlider,
            });
        }
        return pane;
    }

    static createSliderOption(option: Option, key: string, domOption: { mode: string }) {
        const property = option.getProperty(key);
        ...
        const pane = this.createSlider({
                ...
                onui: (ui, opt) => {
                    option.onChange(key, event => {
                            ui.value = opt.valueToSlider(event.value).toString()
                    })
                },
                onchange: (value, domEvent) => {
                    option.set(key, value);
    ...
    static createOuter(pane: HTMLElement, key: string, option: Option, domOption: { mode:string } = { mode: '' }) {
