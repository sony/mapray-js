...
class CheckInput {
    ...
    constructor(viewer) {
        ...
        this.is_Mouse_Up = false;                // マウスがクリックされたか
        this.mouse_Up_Pos = [0, 0];              // クリックされたマウスの位置
    ...
    IsMouseUp(mousePos) {
        if(this.is_Mouse_Up){
            mousePos[0] = this.mouse_Up_Pos[0];
            mousePos[1] = this.mouse_Up_Pos[1];
            return true;
        } else {
            return false;
        }
    ...
    _onMouseUp(event) {
        ...
        if (event.button == 0 /* 左ボタン */) {
            ...
            if (event.shiftKey == false & event.altKey == false) {
                ...
                this.mouse_Up_Pos[0] = event.clientX;
                this.mouse_Up_Pos[1] = event.clientY;
            }
        }
    }
